...
- `.github/workflows/helsesjekk.yml` + `scripts/helsesjekk.sh` – **helse-agent** (daglig cron + manuell): døde interne lenker, wp-rester, hardkodede hemmeligheter, placeholder-CTA-er, noindex-lekkasje, manglende canonical, brutte bilde-referanser, sitemap, bilde-regresjon. Åpner/oppdaterer én GitHub-issue (label `helsesjekk`), lukker den automatisk når grønt. `.github/workflows/helse-autofiks.yml` – **PR-basert reparatør** (kun manuell, merger aldri selv); **inert** til Claude GitHub-app + `ANTHROPIC_API_KEY` er lagt inn.
